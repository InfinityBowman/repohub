...
export class DependencyHealthService extends EventEmitter {
  ...
  private runAudit(cwd: string, pm: 'pnpm' | 'npm' | 'yarn'): Promise<VulnerabilitySummary> {
  ...
  private runOutdated(cwd: string, pm: 'pnpm' | 'npm' | 'yarn'): Promise<OutdatedSummary> {
    return new Promise(resolve => {
      const empty: OutdatedSummary = { major: 0, minor: 0, patch: 0, total: 0 };

      const args =
        pm === 'pnpm' ? ['outdated', '--format', 'json']
        : pm === 'yarn' ? ['outdated', '--json']
        : ['outdated', '--json'];
  ...
  private calculateStatus(vulns: VulnerabilitySummary, outdated: OutdatedSummary): HealthStatus {
