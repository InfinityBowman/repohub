...
export class ScaffoldService extends EventEmitter {
  ...
  createFromTemplate(
    ...
    if (!templatesDir)
      return { success: false, error: 'No templates directory configured' }
  ...
  addRecipe(recipe: ScaffoldRecipe): void {
    ...
    config.hiddenDefaultRecipes = config.hiddenDefaultRecipes.filter(
      (id) => id !== recipe.id,
    )
  ...
  run(
    ...
    try {
      ...
      ptyProcess.onData((data: string) => {
        ...
        const markerMatch = urlScanBuffer.match(
          /@@REPOHUB_OPEN_URL@@(https?:\/\/[^\s\r\n]+)/,
        )
        if (markerMatch) {
