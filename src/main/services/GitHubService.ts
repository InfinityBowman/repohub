...
export class GitHubService extends EventEmitter {
  ...
  async checkAvailability(): Promise<GitHubStatus> {
    ...
    try {
      ...
      this._status = { available: true, authenticated: false, error: 'Not authenticated. Run: gh auth login' }
    }
  ...
  async fetchAllUserPRs(maxRepos?: number): Promise<PRInfo[]> {
    ...
    for (let i = 0; i < repos.length; i += BATCH_SIZE) {
      ...
      const results = await Promise.allSettled(
        batch.map((repo) => this.fetchPRsForRepo(repo)),
      )
  ...
  private async fetchPRsForRepo(repo: { id: string; name: string; path: string; gitBranch?: string }): Promise<PRInfo[]> {
