...
export class CodeSearchService extends EventEmitter {
  ...
  async search(options: SearchOptions): Promise<SearchResult[]> {
    ...
    if (options.directories?.length) {
      ...
      if (scanDir) {
        options.directories = options.directories.filter(
          (dir) => dir.startsWith(scanDir),
        )
      }
    }
    ...
    for (const result of results) {
      ...
      if (options.directories?.length) {
        const matchesDir = options.directories.some((dir) =>
    ...
    try {
      for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
        ...
        const texts = batch.map(
          (c) =>
            `${c.language} ${c.constructType} ${c.constructName}\n${c.code}`,
        )
