...
function ScoreBar({ score }: { score: number }) {
  const colorClass =
    score >= 70 ? 'bg-green-500' : score >= 50 ? 'bg-yellow-500' : 'bg-orange-500'
  return (
...
function ResultCard({ result, onOpenFile }: { result: SearchResult; onOpenFile: (r: SearchResult) => void }) {
    ...
    <Card className="overflow-hidden">
      <CardContent className="p-4">
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Badge variant="secondary">
                {result.chunk.constructType}
              </Badge>
              {result.chunk.constructName !== '<anonymous>' && result.chunk.constructName !== '<block>' && (
            ...
            <button
              ...
              className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors text-left truncate"
            >
        ...
        <div className="relative">
          <pre className="overflow-x-auto rounded-md bg-secondary/50 p-3 text-xs leading-relaxed">
