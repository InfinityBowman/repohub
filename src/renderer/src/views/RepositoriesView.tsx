...
function FolderNode({ node, depth }: { node: TreeNode; depth: number }) {
  ...
  return (
    <div>
      <div
        className='hover:bg-secondary/50 flex items-center justify-between rounded-md pr-2 transition-colors'
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
      >
        <button
          onClick={() => setOpen(!open)}
          className='text-muted-foreground hover:text-foreground flex flex-1 items-center gap-1.5 px-2 py-1.5 text-sm transition-colors'
...
export function RepositoriesView() {
  const { repositories, allRepositories, loading, error, scan, filterText, setFilter } =
    useRepositories();
  ...
  return (
    <div className='flex flex-col gap-4'>
      <div className='flex items-center justify-between'>
        <h2 className='text-foreground text-xl font-semibold'>Repositories</h2>
