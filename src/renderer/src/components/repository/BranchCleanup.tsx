...
export function BranchCleanup({ repoId }: { repoId: string }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          {mergedBranches.length > 0 && (
            <Badge variant="outline" className="border-yellow-800/50 bg-yellow-900/20 text-yellow-400">
      ...
      {branches.length === 0 ? (
        <p className="text-sm text-muted-foreground">No local branches found.</p>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 overflow-hidden">
                  ...
                  {branch.isCurrent && (
                    <Badge variant="outline" className="border-blue-800/50 bg-blue-900/20 text-blue-400 text-[10px] px-1.5 py-0">
                      current
                    </Badge>
                  )}
                  {branch.isMerged && !branch.isCurrent && (
                    <Badge variant="outline" className="border-green-800/50 bg-green-900/20 text-green-400 text-[10px] px-1.5 py-0">
                      merged
                    </Badge>
                  )}
                  {branch.upstream && (
                  ...
                  <span className="text-xs text-muted-foreground">{branch.lastCommit}</span>
                </div>
                {isMergedDeletable && (
                  <TooltipProvider delayDuration={300}>
                    <Tooltip>
