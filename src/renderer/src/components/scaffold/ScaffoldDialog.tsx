...
export function ScaffoldDialog({ open, onOpenChange }: ScaffoldDialogProps) {
  ...
  const handleCreate = async () => {
    ...
    if (selected.type === 'template') {
      ...
      setStep('running');
      const result = await window.electron.scaffold.run(selected.recipe.id, projectName);
      if (!result.success) {
        setExitCode(1);
        setFinishedProjectName(projectName);
        setStep('done');
  ...
  const handleDone = useCallback((data: { exitCode: number; projectName: string }) => {
  ...
  const handleOpenProject = async () => {
    ...
    const newRepo = repos.find(
      (r: any) => r.name === finishedProjectName || r.name.endsWith('/' + finishedProjectName),
    );
    if (newRepo) {
        ...
        <DialogHeader>
          <div className='flex items-center gap-2'>
            ...
            <DialogTitle>
              ...
              {step === 'name' &&
                (selected?.type === 'template' ? selected.template.name : selected?.recipe.name)}
                ...
                <ScrollArea className='max-h-[320px] overflow-y-auto'>
                  <div className='flex flex-col gap-1 pr-3'>
                    {recipes.map(recipe => (
                      <div
                        key={recipe.id}
                        className='hover:bg-secondary/50 flex cursor-pointer items-center justify-between rounded-md px-3 py-2'
        ...
        {step === 'name' && (
          <div className='flex flex-col gap-4'>
            <div className='flex flex-col gap-2'>
              ...
              {copyError && <span className='text-destructive text-xs'>{copyError}</span>}
            </div>

            {selected?.type === 'template' ?
              <div className='bg-secondary/50 text-muted-foreground rounded-md p-3 text-xs'>
                Copy <span className='text-foreground font-medium'>{selected.template.name}</span>{' '}
                template into a new project directory.
              </div>
            : selected?.type === 'recipe' && selected.recipe.url ?
              <div className='bg-secondary/50 text-muted-foreground flex flex-col gap-1 rounded-md p-3 text-xs'>
                <p>
                  This will open{' '}
                  <span className='text-foreground font-medium'>{selected.recipe.url}</span> in your
                  browser.
                </p>
                <p>
                  Configure your project there, then paste the generated command into the terminal.
                </p>
              </div>
            : <div className='bg-secondary/50 rounded-md p-3'>
                <code className='text-muted-foreground text-xs break-all'>{resolvedCommand}</code>
              </div>
            }
              ...
              <div className='flex items-center justify-between'>
                {selected?.type === 'recipe' && selected.recipe.url ?
                  <span className='text-muted-foreground text-xs'>
                    Paste the command from your browser. Type{' '}
                    <kbd className='bg-muted rounded px-1 py-0.5 text-[10px] font-medium'>exit</kbd>{' '}
                    when done.
                  </span>
                : <span />}
